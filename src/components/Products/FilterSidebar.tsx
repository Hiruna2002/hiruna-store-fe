...
/* Keys grouped by value type */
type ArrayFilterKeys = "size" | "material" | "brand";
type ScalarFilterKeys = "category" | "gender" | "color";
...
const FilterSidebar:FC = () => {
  ...
  useEffect(() => {
    ...
    setPriceRange([
      0,
      params.maxPrice ? Number(params.maxPrice) : 100000,
    ]);
  }, [searchParams]);
  ...
  const handleFilterChange = (
    ...
    if (type === "checkbox") {
      const key = name as ArrayFilterKeys;

      newFilters[key] = checked
        ? [...newFilters[key], value]
        : newFilters[key].filter((item) => item !== value);
    } else {
      const key = name as ScalarFilterKeys;
      newFilters[key] = value;
    }
  ...
  const updateURLParams = (newFilters: Filters): void => {
    const params = new URLSearchParams();
